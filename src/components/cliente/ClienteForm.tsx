    ...
    <form onSubmit={(e) => e.preventDefault()}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input
                    ...
                    onChange={(e) => setFormData({ ...formData, dataNascimento: new Date(e.target.value) })}
                />
            </div>
            <div>
                <label htmlFor="dataCadastro" className="block text-sm font-medium text-gray-700">Data de Cadastro</label>
                <input
                    type="date"
                    id="dataCadastro"
                    className="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500"
                    value={formData.dataCadastro.toISOString().split('T')[0]}
                    onChange={(e) => setFormData({ ...formData, dataCadastro: new Date(e.target.value) })}
                />
            </div>
        </div>
        ...
        <div className="space-y-4">
            {formData.telefones.map((telefone, index) => (
                <div key={telefone.id} className="p-4 border rounded-lg bg-gray-50">
                    <label htmlFor={`telefone-${telefone.id}`} className="block text-sm font-medium text-gray-700">Telefone {index + 1}</label>
        ...
        <div className="space-y-4">
            {formData.documentos.map((documento, index) => (
                <div key={documento.id} className="p-4 border rounded-lg bg-gray-50 space-y-3">
                    ...
                    <div>
                        <label htmlFor={`doc-numero-${documento.id}`} className="block text-sm font-medium text-gray-700">Número</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label htmlFor={`doc-tipo-${documento.id}`} className="block text-sm font-medium text-gray-700">Tipo</label>
                        <select
                    ...
                    <div>
                        <label htmlFor={`doc-data-${documento.id}`} className="block text-sm font-medium text-gray-700">Data de Expedição</label>
                        <input
                            type="date"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    onChange={(e) => setFormData({ ...formData, endereco: { ...formData.endereco, pais: e.target.value } })}
                />
            </div>
            <div>
                ...
                <input
                    ...
                    onChange={(e) => setFormData({ ...formData, endereco: { ...formData.endereco, codigoPostal: e.target.value } })}
                />
            </div>
        </div>
    </form>
