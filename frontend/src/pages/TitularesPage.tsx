...
const TitularesPage: React.FC = () => {
  ...
  const renderContent = () => {
    if (isLoading) {
      return <div className="text-center p-10 text-gray-500">Carregando clientes...</div>;
    }
    if (error) {
      return <div className="text-center p-10 text-red-600 bg-red-50 rounded-lg"><strong>Erro:</strong> {error}</div>;
    }
    if (clientes.length === 0) {
        return <div className="text-center p-10 text-gray-500">Nenhum cliente titular encontrado.</div>;
    }
    return (
        <ClienteTable
          clientes={clientes}
